...
//modification
CloseHandle(_hSemCreator); 
CloseHandle(_hSemClient);
//end modification
}
...
sc_simcontext::simulate( const sc_time& duration )
{
	...
	//clock_t start, finish, start1, finish1, start3, finish3, start_ipc, finish_ipc; 
...
//--------------------------------------------------------------------------
// Modification : pause SC every simulation cycle and pass time to simulink
//--------------------------------------------------------------------------
	...
	if(MIXED_SIGNAL)
	{
		...
		if (UE == true)
		{
			if (SwitchFlag)
			{
				SwitchFlag = false;
				c++;
				if(c > 1)   // at the start it is not necessary to release the client (simulink)
				{
					c = 2;  // to avoid the depassement of "int" definition 
					if (!ReleaseSemaphore( 
					_hSemClient,  // handle to semaphore
					1,           // increase count by one
					NULL) )      // not interested in previous count
					{
						cout <<"Can not release Client sem"<<endl;
					}
				}
							
				WaitForSingleObject( 
				_hSemCreator,   // handle to semaphore
		...
		}else 
		{
			if ( FS == true)
			{
				...
				nextTime = next_time().to_seconds();

				// If time is sampling time, set to sample data
				double diff = nextTime/Speriod_0;
				if( (diff - floor(diff)) < 1e-6 )
				{
					// pass next time (in Second) to simulink through shared memory
					*(((double*)MemPtr + 200)) = nextTime;
				// Else check for events
				}else {
					*(((double*)MemPtr + 100)) = nextTime; 
				}
				
			//start_ipc = clock(); //to compute overhead_ipc
				c++;
				if(c > 1)   // at the start it is not necessary to release the client (simulink)
				{
					c = 2;  // to avoid the depassement of "int" definition 
...
//insert state event in the queu
// indications are located from 212, time events are located from 102 
		//start3 = clock(); //to compute overhead_ipc
		for (int i=0;i<10;i++)
		{
			if (*(((double*)MemPtr + 112 + i))== 1)
			{
				*(((double*)MemPtr+112+i)) = 0;
				double TimeEvent = *(((double*)MemPtr+102+i));
				//printf("=========== TimeEvent %.15f\n",TimeEvent); 
				double notif = TimeEvent - sc_time_stamp().to_seconds();
				et_mat[i]->notify(notif, SC_SEC);
			}
		}
		//finish3 = clock(); overhead = overhead + (finish3 - start3);
