...
//Modification to compute overhead
#include <time.h> 
//End of Modification
...
//Modification for mixed-signal
	bool MIXED_SIGNAL, SwitchFlag, FS, UE;
	HANDLE _hSemClient, _hSemCreator; //Handle to the semaphores : used to pause SC every simulation cycle
	double Speriod_0, Speriod_1;
	LPVOID MemPtr;  //pointer to shared memory to pass time to simulink 
	sc_event* et_mat[10]; //pointer to  state events
	//clock_t  overhead, overhead_ipc, ipc_at_start; //to compute overhead
//End modification
    sc_simcontext();
    ~sc_simcontext();
